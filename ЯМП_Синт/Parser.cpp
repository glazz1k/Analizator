...
void Parser::addDeclaredVariableWithType(const string& varName, const string& type) // Используется для добавления переменных, у которых известен тип (int, double)
{
    ...
    {
        declaredVariables->insert(varToken);
        variableTypes[varName] = type;  // Ключ - имя переменной, значение - ее тип ("int" или "double")
    }
}
...
string Parser::getExpressionType()  // Определение типа выражения
{
    ...
    {
        if (variableTypes.find(token) != variableTypes.end())   // Если токен является объявленной переменной
            typeStack.push(variableTypes[token]);               // Извлекаем тип переменной из таблицы типов и помещаем в стек
...
{
    if (variableTypes.find(varName) == variableTypes.end()) // Проверяем, известен ли тип переменной
        return; // Ошибка уже обработана

    string varType = variableTypes[varName];    // Извлекаем тип переменной из таблицы типов
...
{
    if (variableTypes.find(returnVar) == variableTypes.end())   // Проверяем, объявлена ли переменная
    {
        string errorMsg = "строка " + to_string(currentToken.getLine()) +
    ...
    string returnType = variableTypes[returnVar];   // Извлекаем тип переменной из таблицы типов
